...
export function InsightsView({
    ...
    return (
      <div className="mt-4 flex flex-col items-center justify-center rounded-lg border border-border border-dashed bg-background/50 p-16 text-center">
        ...
        <p className="mt-2 max-w-sm font-light text-muted-foreground text-xs leading-relaxed">
          Registra al menos una partida para que el Coach Táctico analice vuestro historial y genere briefing operativo, alertas de fatiga y recomendaciones de roles.
        </p>
      </div>
    );
  }
  ...
  const briefing = useMemo(() => {
    ...
    const avgPlacement = last3.reduce((sum, m) => sum + (m.placement || 0), 0) / 3;

    if (avgPlacement <= 5.0) {
      return {
        type: "success",
        title: "Briefing Táctico: ¡Racha de Éxito! 🏆",
        text: "El escuadrón está dominando el mapa. Tienen un puesto promedio de #" + avgPlacement.toFixed(1) + " en las últimas 3 rondas. Sigan con el juego coordinado, presionen en combate de mediano alcance y mantengan la composición de clases activa."
      };
    } else if (avgPlacement >= 15.0) {
      return {
        type: "warning",
        title: "Briefing Táctico: Ajuste de Emergencia ⚠️",
        text: "Caídas rápidas consecutivas detectadas (puesto promedio #" + avgPlacement.toFixed(1) + "). Sugerencia del Coach: Evitar caídas calientes en POIs céntricos. Despliéguense en la periferia profunda del mapa (ej. Golf Course o Boutique District) y tómense los primeros 5 minutos estrictamente para saquear y asegurar recursos clave antes de confrontar."
      };
    } else {
      return {
        type: "info",
  ...
  const fatigueAlert = useMemo(() => {
    ...
    if (gameCount > 0) {
      ...
      if (lastMatch && lastMatch.player_match_stats && lastMatch.player_match_stats.length > 0) {
        const sumMental = lastMatch.player_match_stats.reduce((sum, p) => sum + (p.mental_state || 5), 0);
    ...
    if (isFatigued || isTilted) {
      return {
        ...
        avgMental,
        gameCount,
        title: isTilted ? "🚨 PROTOCOLO DE DESCANSO ACTIVADO (TILT)" : "⚠️ ADVERTENCIA DE FATIGA MENTAL",
        text: isTilted
  ...
  const squadCompositions = useMemo(() => {
    const comps: Record<string, { count: number; sumPlacement: number; wins: number }> = {};
    ...
    return calculated.sort((a, b) => b.winRate - a.winRate || a.avgPlacement - b.avgPlacement);
  }, [matches]);
  ...
  const roleRecommendations = useMemo(() => {
    ...
    const playerStats: Record<string, Record<string, { kills: number; deaths: number; count: number; wins: number }>> = {};
    ...
    for (const member of squad.members) {
      ...
      const prefStats = stats[prefClass] || { kills: 0, deaths: 0, count: 0, wins: 0 };
      const prefKDR = prefStats.deaths > 0 ? prefStats.kills / prefStats.deaths : prefStats.kills;
      ...
      for (const [cls, cStat] of Object.entries(stats)) {
        if (cls === prefClass) continue;
        if (cStat.count < 2) continue; // Minimum 2 matches played to recommend

        const altKDR = cStat.deaths > 0 ? cStat.kills / cStat.deaths : cStat.kills;
  ...
  return (
    <div className="fade-in-50 animate-in space-y-6 duration-300">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        {/* Briefing panel */}
        <div className={`lg:col-span-2 rounded-lg border p-5 relative overflow-hidden ${
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-lg border border-border bg-card p-5">
          <h3 className="mb-4 border-border/40 border-b pb-3 font-bold text-foreground text-xs uppercase tracking-wider flex items-center gap-1.5 font-mono">
          ...
          {squadCompositions.length === 0 ? (
            ...
            <div className="space-y-4">
              <p className="text-[11px] text-muted-foreground font-light leading-relaxed">
                Análisis histórico de las combinaciones de clase que resultan en mejor posicionamiento y porcentaje de victorias:
              </p>
              <div className="divide-y divide-border/40">
                {squadCompositions.slice(0, 3).map((comp, idx) => (
                  <div key={comp.composition} className="py-3 flex items-center justify-between gap-4 first:pt-0 last:pb-0">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-muted-foreground font-light">
                        Jugada en {comp.count} {comp.count === 1 ? "ronda" : "rondas"}
                      </p>
                    </div>
            ...
            {roleRecommendations.length === 0 ? (
              <div className="py-8 text-center">
                <p className="font-light text-muted-foreground text-xs">
                  ✅ Todos los operadores juegan en coherencia con su rendimiento estadístico óptimo.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                <p className="text-[11px] text-muted-foreground font-light leading-relaxed">
                  Operadores que registran una efectividad de combate (KDR) significativamente mayor en un rol diferente a su clase favorita preestablecida:
                </p>
                <div className="space-y-3.5">
                  {roleRecommendations.map((rec) => (
                    <div key={rec.gamertag} className="rounded-md border border-border bg-background p-3.5 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-foreground">
                          {cleanGamertag(rec.gamertag)}
                        </span>
                        <div className="flex items-center gap-1 font-mono text-[10px]">
                          <span className="text-muted-foreground">{rec.preferredClass}</span>
                          <span className="text-muted-foreground/40">➔</span>
                          <span className="text-emerald-400 font-bold">{rec.recClass}</span>
                        </div>
                      </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        {dropAnalysis.dropGanador && (
          ...
          >
            ...
            <p className="mt-1 font-light text-muted-foreground text-xs leading-relaxed max-w-sm">
              Esta zona de despliegue inicial provee la tasa de supervivencia promedio más alta y la mayor estabilidad estratégica al inicio de la ronda.
            </p>
            <div className="mt-4 flex items-center justify-between text-xs font-mono text-muted-foreground">
              <span>Puesto Promedio: <span className="font-bold text-emerald-400">#{dropAnalysis.dropGanador.avgPlacement.toFixed(1)}</span></span>
              <span>Despliegues: <span className="font-bold text-foreground">{dropAnalysis.dropGanador.count}</span></span>
            </div>
        ...
        {dropAnalysis.rutaMuerte && (
          ...
          >
            ...
            <p className="mt-1 font-light text-muted-foreground text-xs leading-relaxed max-w-sm">
              Zonas con alta incidencia de wipes de escuadrón tempranos o bajo posicionamiento final. Se aconseja evitar en el briefing de despliegue.
            </p>
            <div className="mt-4 flex items-center justify-between text-xs font-mono text-muted-foreground">
              <span>Puesto Promedio: <span className="font-bold text-destructive">#{dropAnalysis.rutaMuerte.avgPlacement.toFixed(1)}</span></span>
              <span>Despliegues: <span className="font-bold text-foreground">{dropAnalysis.rutaMuerte.count}</span></span>
            </div>
