!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{};e.SENTRY_RELEASE={id:"46f9c79581e3621fd00edd9e90763fed8129f248"};var n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="329ae1b0-f94c-464e-a423-3ef482e9f3de",e._sentryDebugIdIdentifier="sentry-dbid-329ae1b0-f94c-464e-a423-3ef482e9f3de");}catch(e){}}();import './server_C8rEq11W.mjs';
...
import { B as BUSINESS_CONFIG } from './business_Bhtk-lpw.mjs';
import { d as defineAction, A as ActionError } from './entrypoint_BMrEzvwW.mjs';
  ...
  squad: {
    create: defineAction({
      ...
      input: z.object({
        ...
        ).length(z.number().min(1).max(8).default(4)),
      ...
      handler: async (input, context) => {
        ...
        const otherMembers = input.members.filter((m) => m.slot_number !== 1);
        if (otherMembers.length) {
          const { error: otherError } = await supabase.from("squad_members").insert(otherMembers.map((m) => ({
            squad_id: squad.id,
            gamertag: m.gamertag || "",
            level: m.level || 1,
            favorite_class: m.favorite_class,
          ...
          if (otherError) {
            console.error("Error creating other members:", otherError);
            await supabase.from("squads").delete().eq("id", squad.id);
            throw new ActionError({
              code: "INTERNAL_SERVER_ERROR",
              message: `Error al registrar integrantes: ${otherError.message}`
            });
          }
        }
    ...
    claimSlot: defineAction({
      ...
      handler: async (input, context) => {
        ...
        const { data: squadInfo, error: squadInfoError } = await supabase.from("squads").select(`slot_count, access_code`).eq("id", input.squadId).single();
        if (squadInfoError) {
          throw new ActionError({
            code: "INTERNAL_SERVER_ERROR",
